...
export default function StudentDashboard() {
  ...
  const dash = (student.overallCompetency / 100) * circ
    ...
    <div className="space-y-6 page-enter">
      ...
      <div className="relative bg-gradient-to-r from-indigo-600 via-indigo-700 to-blue-700 rounded-2xl overflow-hidden shadow-card-md">
        ...
        <div className="relative p-5 sm:p-6">
          {/* Top row: text + ring (ring moves below on xs) */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            {/* Text block */}
            <div className="min-w-0">
              <p className="text-indigo-200 text-sm font-medium mb-0.5">{greeting} 👋</p>
              <h1 className="text-xl sm:text-2xl font-bold text-white truncate">{student.name}</h1>
              <p className="text-indigo-300 text-xs sm:text-sm mt-0.5 truncate">
                {student.institution} · {student.course} · Year {student.year}
              </p>
              <div className="flex items-center gap-2 mt-3 flex-wrap">
                <Badge className="bg-white/20 text-white border-white/20 text-xs">Step 3 of 6 — Matching</Badge>
                <Badge className="bg-indigo-500/40 text-indigo-100 border-indigo-400/30 text-xs" dot>Skill Loop Active</Badge>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 flex-shrink-0">
              ...
              <div className="relative w-20 h-20 sm:w-28 sm:h-28 flex-shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-xl sm:text-2xl font-extrabold text-white tabular">{student.overallCompetency}%</span>
                  <span className="text-[8px] sm:text-[9px] text-indigo-200 font-medium">Readiness</span>
                </div>
              </div>

              {/* Action buttons — hidden on xs, visible sm+ */}
              <div className="hidden sm:flex flex-col gap-2.5">
                <Link href="/student/assessments">
                  <Button size="sm" className="bg-white text-indigo-700 hover:bg-indigo-50 shadow whitespace-nowrap">
                    <ClipboardCheck size={13} /> Reassess
                  </Button>
                </Link>
                <Link href="/student/opportunities">
                  <Button size="sm" className="bg-indigo-500/40 text-white border border-white/30 hover:bg-indigo-500/60 whitespace-nowrap">
          ...
          <div className="flex gap-2 mt-4 sm:hidden">
            <Link href="/student/assessments" className="flex-1">
              <Button size="sm" className="bg-white text-indigo-700 hover:bg-indigo-50 shadow w-full justify-center">
                <ClipboardCheck size={13} /> Reassess
              </Button>
            </Link>
            <Link href="/student/opportunities" className="flex-1">
              <Button size="sm" className="bg-indigo-500/40 text-white border border-white/30 hover:bg-indigo-500/60 w-full justify-center">
                <Briefcase size={13} /> Opportunities
              </Button>
            </Link>
          </div>
      ...
      {/* ── Skill profile + side cards ────────────────────────────────────── */}
      {/* On mobile: single column stack. On lg: 3-col grid */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">

        {/* Skill profile — spans 2 of 3 cols on desktop */}
        <Card className="lg:col-span-2">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-base font-semibold text-gray-900">Skill Profile</h2>
              <p className="text-xs text-gray-400 mt-0.5">Latest assessed competencies</p>
            </div>
            <Link href="/student/skills">
              <Button variant="ghost" size="sm">Full Profile <ArrowRight size={13} /></Button>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1">
                ...
                <div key={skill.id} className="mb-3.5">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      {skill.trend === 'up' && <TrendingUp size={11} className="text-emerald-500 flex-shrink-0" />}
                    </div>
            ...
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 mt-4 sm:mt-0">Soft Skills</p>
              {soft.map((skill) => (
                <div key={skill.id} className="mb-3.5">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-gray-700 truncate min-w-0">{skill.name}</span>
                    <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                      <span className="text-xs text-gray-400 tabular">{skill.score}%</span>
                      <Badge
                        variant={skill.level === 'Advanced' ? 'indigo' : skill.level === 'Intermediate' ? 'info' : 'default'}
        ...
        <div className="space-y-5">
              ...
              {mockSkillGaps.map((gap) => (
                <div key={gap.skillId} className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-gray-800 truncate min-w-0 mr-2">{gap.skillName}</span>
                    <Badge variant={gap.priority === 'High' ? 'danger' : 'warning'} size="xs" className="flex-shrink-0">
                      {gap.priority}
                    </Badge>
                  </div>
          ...
          <Card tint="indigo" className="border-indigo-100">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {[
                { step: 1, label: 'Assess',  done: true,  active: false },
                { step: 2, label: 'Gaps',    done: true,  active: false },
                { step: 3, label: 'Match',   done: false, active: true  },
                { step: 4, label: 'Learn',   done: false, active: false },
                { step: 5, label: 'Improve', done: false, active: false },
                { step: 6, label: 'Repeat',  done: false, active: false },
              ].map((s) => (
                <div key={s.step}
                  className={`rounded-lg p-1.5 text-center text-[10px] font-semibold transition-all ${
            ...
            <p className="text-[10px] text-indigo-500 mt-2.5 text-center leading-relaxed">
              Step 3: Finding opportunities that fit your profile
            </p>
          </Card>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {recommendedActions.map((action) => (
            <Link key={action.id} href={action.href}>
              <div className="flex items-start gap-3 p-3 sm:p-3.5 rounded-xl hover:bg-gray-50 border border-transparent hover:border-gray-100 transition-all cursor-pointer group">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-medium text-gray-800 leading-snug">{action.label}</span>
                  {/* Badges below label on mobile, inline on sm+ */}
      ...
      {/* ── Reassess nudge ────────────────────────────────────────────────── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-gradient-to-r from-indigo-50 to-blue-50 border border-indigo-100 rounded-2xl">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-indigo-600 mt-0.5">Last assessed 3 weeks ago · Reassessing improves your opportunity matches</p>
          </div>
        </div>
        <Link href="/student/assessments" className="sm:flex-shrink-0">
          <Button variant="primary" size="sm" className="w-full sm:w-auto">Reassess Now</Button>
        </Link>
      </div>
    </div>
  )
